...
public class LiteDbRepository<T> : DisposableBase, IRepository<T>
{
    ...
    private static LiteDatabase GetConnection(GlobalSettings settings)
    {
    ...
    public LiteDbRepository(GlobalSettings settings, ILiteDbModel<T>? model = null)
    {
        Database ??= LiteDbRepository<T>.GetConnection(settings);
    ...
    public IEnumerable<T> SelectAll(int skip = 0, int limit = int.MaxValue)
    {
        ...
        return Collection.Query().Skip(skip).Limit(limit).ToEnumerable();
    }
