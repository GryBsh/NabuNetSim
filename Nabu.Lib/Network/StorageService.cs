...
public class StorageService(    ILogger<StorageService> console,     GlobalSettings settings,     ISourceService sources,    LocationService location)
{
    ...
    public string StorageRoot { get; } = location.Storage;     public string StorageSource { get; } = location.StorageSource;
    ...
    public void AttachStorage(AdaptorSettings settings, string name)
    {
        ...
        var source = location.StorageSource; //SourceFolder(root.FullName);
    ...
    {
        var path = location.Programs;

        if (!Path.Exists(path))
            Directory.CreateDirectory(path);

        var storage = StorageSource;
    ...
    private static string AdaptorFolderName(string name)         => $"{AdaptorFolderNamePrefix}{name}";
