...
{
    public class RetroNetTelnetProtocol : Protocol<AdaptorSettings>
    {
        public RetroNetTelnetProtocol(ILogger<RetroNetTelnetProtocol> logger) : base(logger)
        {
        }

        public override byte[] Messages { get; } = [0xA6];

        public override byte Version => 0x01;

        private void Write(string message) => base.Write(NabuLib.FromASCII(message).ToArray());
        ...
        private string ReadLine(bool echoOff = false, char? replacement = null)
        {
            ...
            while (true)
            {
                var incoming = Encoding.ASCII.GetChars(Read(1));
