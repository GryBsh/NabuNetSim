...
namespace Nabu.Protocols.RetroNet
{
    ...
    {
        private static ConcurrentBag<int> ServerPorts { get; } = [];
        private Dictionary<byte, IRetroNetTcpClientHandle> Clients { get; } = [];
        private List<Socket> Connected { get; } = [];
                        ...
                        async () =>
                        {
                            while (Server is not null)
                            {
                                ...
                                catch (Exception ex)
                                {
                                    ...
                                    Logger.LogWarning(ex, "TCP Server Failed");
                                }
                            }
                        }
                    );
        ...
        private bool ShutdownTCPServer()
        {
            ...
            Logger.LogWarning($"Shutting down TCP Server");
            ...
            Adapter.TCPServerActive = false;
            Adapter.TCPServerProtocol = string.Empty;
            Adapter.TCPServerPort = 0;
        ...
        private bool StartTCPServer(ProgramSource source)
        {
            ...
            if (source.TCPServerPort is not null and not 0)
            {
                ...
                {
                    Logger.LogDebug($"TCP Port `{port}` is unavailable");
                    return false;
                }
            }
            ...
            if (port == default)
            {
                Logger.LogWarning($"TCP Server Ports Exhausted");
                return false;
            }
            Adapter.TCPServerProtocol = nameof(RetroNetProtocol);
            Adapter.TCPServerPort = port;

            Server = NabuLib.Socket();
            Server.Bind(new IPEndPoint(IPAddress.Any, port));
            Server.Listen();
            Adapter.TCPServerActive = true;
            Logger.LogInformation($"RetroNet TCP Server Started on port {port}");
