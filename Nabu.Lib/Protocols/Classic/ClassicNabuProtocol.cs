...
namespace Nabu.Protocols.Classic
{
    [Runtime.Invisible]
    public class ClassicNabuProtocol : Protocol<AdaptorSettings>
    {
        ...
        public override byte[] Messages { get; } = [0x83];
        ...
        private void GetStatus()
        {
            ...
            switch (data)
            {
                case StatusMessage.Signal: // <- NA Channel Lock Status
                    if (ChannelKnown)
                    {
                        Log($"NPC: {nameof(StatusMessage.Signal)}? | NA: {nameof(Status.Good)}");
                ...
                case StatusMessage.Subscription: // <-- NPC Program/DOS Loaded
                    ...
                    if (Network.Source(Adapter!)?.EnableExploitLoader is true)
                    {
        ...
        private async Task SegmentRequest()
        {
                ...
                pak == Message.TimePak)
            {
                ...
                Log(NabuLib.FormatSeparated('|', time.ToArray()));
                SendPacket(pak, time, time.Length, last: true);
                return;
            }

            // Network Emulator
            var (type, segmentData) = await Network.Request(Adapter!, pak);
        ...
        private void SetStatus()
        {
            ...
            {
                case Status.None:
                    Log($"NPC: Status: Idle ({FormatSeparated(status)})");
                    break;

                case Status.Good:
                    Log($"NPC: Status: All Good ({FormatSeparated(status)})");
                    break;

                default:
                    Log($"NPC: Status: {FormatSeparated(status)}");
                    break;
            }
        ...
        private void SendPacket(int pak, Memory<byte> buffer, int totalLength, bool last = false)
        {
            ...
            if (last)
            {
                ...
                Network.UnCachePak(Adapter!, pak);
                ...
                Started = null;
                var source = Network.Source(Adapter!);
                if (Adapter?.ReturnToSource is not null &&
                    (pak is not 1 || source?.EnableExploitLoader is false)
                )
                {
                    Adapter.Source = Adapter.ReturnToSource!;
                    Adapter.Program = Adapter.ReturnToProgram;
                    Adapter.ReturnToSource = null;
                    Adapter.ReturnToProgram = null;
                }
            }
        }
        ...
        public override bool Attach(AdaptorSettings settings, Stream stream)
        {
            ...
            Channel = Adapter!.AdapterChannel;
